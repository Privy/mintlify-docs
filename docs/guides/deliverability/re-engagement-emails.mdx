...
Privy has a **Contact Re-Engagement** template that you can use. When you create a new email and [select a template](https://privy.mintlify.app/docs/learn/email-campaigns/email-templates#choose-a-template), you can find it in the *Privy Templates* folde&#x72;*.*
...
* Are you there, \{\{ contact.first\_name }}?&#x20;
...
The easiest segment to send a Re-Engagement Email to is your [Unengaged Contacts](https://privy.mintlify.app/docs/learn/email-campaigns/segmentation#unengaged-contacts) Smart Segment. These are mailable contacts that have been on your list for over 90 days, but have not opened a single email in 3+ months.

However, you can also [create a Custom Segment](https://privy.mintlify.app/docs/learn/email-campaigns/segmentation#custom-segments) to send to if you'd like more control over the criteria.

If you create a Custom Segment, the 2 essential criteria you'll want to add are:

1. Email Engagement: **Did not open** + **in the last** + **`[value]` days**

2. Created at: **is before** + **`[date]`**
...
1. Order count: if you want to focus on contacts that have never ordered with you before, you can add **Order count** + **is** + **0**.

2. Last order: if you want to exclude contacts who may have ordered after a certain date, you can add **Last order** + **is after** + **`[date]`**.
...
Head to your [Campaigns](https://dashboard.privy.com/message-campaigns) page and click into your Re-Engagement email to get an in-depth view of [your campaign's performance](https://privy.mintlify.app/docs/learn/email-campaigns/email-campaign-reporting#individual-campaign-reporting), including its delivery, engagement, and activity.

### Removing non-openers

To remove any contacts who still failed to engage with this email, scroll down to your campaign's [*Activity* table](https://privy.mintlify.app/docs/learn/email-campaigns/email-campaign-reporting#activity) and **Export** a CSV of your recipients. You will receive an email with a link to download your file.

Make sure your filter and remove any contacts who opened your email, then [import the rest as unsubscribed](https://privy.mintlify.app/docs/learn/contact-management/unsubscribe-contacts#bulk-unsubscribe).